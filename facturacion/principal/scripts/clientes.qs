...
/////////////////////////////////////////////////////////////////
//// KLO /////////////////////////////////////////////////////
class klo extends diasPago 
{
	var lblNombreClienteFac:Object;
	
	function klo( context ) { diasPago( context ); } 
    	function init() { this.ctx.klo_init(); }
	function comprobarBcoPredeterminado() { this.ctx.klo_comprobarBcoPredeterminado(); }
	function buscarCliente() {
		this.ctx.klo_buscarCliente();
	}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function klo_init()
{
	...
	this.iface.__init();
	this.iface.comprobarBcoPredeterminado();
	
	this.iface.lblNombreClienteFac = this.child("lblNombreClienteFac");
	connect(this.child("tbnBuscarCliente"), "clicked()", this, "iface.buscarCliente()");
	
	this.iface.lblNombreClienteFac.text = util.sqlSelect("clientes", "nombre", "codcliente = '" + cursor.valueBuffer("codclifac") + "'");
}

function klo_comprobarBcoPredeterminado()
{
	var curCBC:FLSqlCursor = this.child("tdbCuentas").cursor();
	curCBC.select();
	
	if (curCBC.size() == 1)
		this.iface.cambiarCuentaDom();
}

function klo_buscarCliente()
{
	var ruta:Object = new FLFormSearchDB("buscliente");
	var curClientes:FLSqlCursor = ruta.cursor();
	var cursor:FLSqlCursor = this.cursor();
	var util:FLUtil = new FLUtil();
	var codclifac:String;
	
	ruta.setMainWidget();
	codclifac = ruta.exec("codcliente");
	if (!codclifac) return;
	
	cursor.setValueBuffer("codclifac", codclifac);
	
	this.iface.lblNombreClienteFac.text = util.sqlSelect("clientes", "nombre", "codcliente = '" + codclifac + "'");
}
