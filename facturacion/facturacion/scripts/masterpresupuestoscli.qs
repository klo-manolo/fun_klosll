...
class klo extends ivaIncluido {
	...
	function datosLineaPedido(curLineaPresupuesto:FLSqlCursor):Boolean {
		return this.ctx.klo_datosLineaPedido(curLineaPresupuesto);
	}
}
...
@return	True si la copia se realiza correctamente, false en caso contrario
\end */
function klo_datosLineaPedido(curLineaPresupuesto:FLSqlCursor):Boolean
{
	if(!this.iface.__datosLineaPedido(curLineaPresupuesto))
		return false;

	with (this.iface.curLineaPedido) {
		setValueBuffer("coste", curLineaPresupuesto.valueBuffer("coste"));
	}
	return true;
}
