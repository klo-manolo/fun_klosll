...
/////////////////////////////////////////////////////////////////
//// KLO /////////////////////////////////////
class klo extends dtoEspecial {
	var arrayLineas_:Array;
    function klo( context ) { dtoEspecial ( context ); }
	function init() {
...
function klo_init()
{
	...
	if (!this.iface.arrayLineas_) {
		MessageBox.warning(util.translate("scripts", "Error al cargar los datos de las líneas"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
}
...
function klo_cargarArrayLineas():Array
{
	...
	qryLineas.setTablesList("albaranesprov,lineasalbaranesprov");
	qryLineas.setSelect("a.codalmacen, a.fecha, la.referencia, SUM(la.cantidad)");
	qryLineas.setFrom("albaranesprov a INNER JOIN lineasalbaranesprov la ON a.idalbaran = la.idalbaran");
	qryLineas.setWhere("a.idalbaran = " + cursor.valueBuffer("idalbaran") + " AND la.referencia IS NOT NULL GROUP BY a.codalmacen, a.fecha, la.referencia");
	...
	while (qryLineas.next()) {
		arrayLineas[i] = [];
		arrayLineas[i]["idarticulo"] = qryLineas.value("la.referencia");
		arrayLineas[i]["codalmacen"] = qryLineas.value("a.codalmacen");
		arrayLineas[i]["fecha"] = qryLineas.value("a.fecha");
		arrayLineas[i]["cantidad"] = qryLineas.value("SUM(la.cantidad)");
		i++;
	}
	return arrayLineas;
}
...
{
	debug("KLO--> controlCabeceraKlo");
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor();
	
	var arrayActual:Array = this.iface.cargarArrayLineas();
	if (!arrayActual) {
		MessageBox.warning(util.translate("scripts", "Error al cargar los datos de artículo"), MessageBox.Ok, MessageBox.NoButton);
		return false;
	}
	...
	for (var i:Number = 0; i < arrayAfectadas.length; i++) {
		if (!flfactalma.iface.pub_calculosUltCMS(arrayAfectadas[i]["idarticulo"], arrayAfectadas[i]["codalmacen"])) {
			return false;
		}
		flfactalma.iface.pub_cambiarCosteMedio(arrayAfectadas[i]["idarticulo"]);
		flfactalma.iface.pub_cambiarUltimoCoste(arrayAfectadas[i]["idarticulo"]);
