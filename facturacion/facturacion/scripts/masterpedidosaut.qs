...
class klo extends oficial 
{
	...
	function copiaLineas(idPedidoAut:Number, idPedido:Number):Boolean {
		return this.ctx.klo_copiaLineas(idPedidoAut, idPedido);
	}
	function filtrarTabla():Boolean {
		return this.ctx.klo_filtrarTabla();
	}
	function filtroTabla():String {
		return this.ctx.klo_filtroTabla();
	}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function klo_filtrarTabla():Boolean
{
...
function klo_filtroTabla():String
{
	var util:FLUtil = new FLUtil();
	var filtro:String = "";
	var codEjercicio:String = flfactppal.iface.pub_ejercicioActual();
	if (codEjercicio) {
		var fechaInicio:Date = util.sqlSelect("ejercicios", "fechainicio", "codejercicio = '"+codEjercicio+"'");
		var fechaFin:Date = util.sqlSelect("ejercicios", "fechafin", "codejercicio = '"+codEjercicio+"'");
		filtro = "fecha >= '" + fechaInicio + "' AND fecha <= '"+fechaFin+"'";
	}
	return filtro;
}

function klo_copiaLineas(idPedidoAut:Number, idPedido:Number):Boolean
{
	var codProveedor:String = this.iface.curPedido.valueBuffer("codproveedor");
	var referencia:String;
	var variacion:Number;	
	...
	while (curLineaPedidoAut.next()) {
		...
		if (!this.iface.copiaLineaPedidoAut(curLineaPedidoAut, idPedido))
			return false;
		//KLO 040308 if (!flfacturac.iface.pub_cambiarStockOrd(referencia, variacion))
		//	return false;
	}
	return true;
}
