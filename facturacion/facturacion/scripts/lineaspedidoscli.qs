...
class klo extends ivaIncluido 
{
	...
	//	return this.ctx.klo_habilitarPorIvaIncluido();
	//}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function klo_commonCalculateField(fN:String, cursor:FLSqlCursor):String
{
	...
	var referencia:String = cursor.valueBuffer("referencia");
	var codAlmacen:String = util.sqlSelect(tablaPadre, "codalmacen", wherePadre);
	var fechaFin:Date = util.sqlSelect(tablaPadre, "fecha", wherePadre);
	
	switch (fN) {
	
		case "costeultimo":
			//valor = util.sqlSelect("articulos", "costeultimo", "referencia = '" + referencia + "'");
			//valor = flfactalma.iface.pub_CMSenUnaFecha(referencia, codAlmacen, fechaFin);
			valor = flfactalma.iface.pub_costeEnUnaFecha(referencia, codAlmacen, fechaFin);
			debug("valor: "+valor);
			break;
		
		case "pvpunitarioiva":
			valor = this.iface.commonCalculateField("pvpunitario", cursor);
			break;
		...
		case "pvpunitario":
			...
			if (cursor.valueBuffer("ivaincluido")) {
				cursor.setValueBuffer("pvpunitarioiva",valor);
...
function klo_commonBufferChanged(fN:String, miForm:Object)
{
	...
	switch (fN) {
		...
		case "referencia":
			...
			miForm.cursor().setValueBuffer("coste",this.iface.commonCalculateField("costeultimo", miForm.cursor()));
			break;
		default:
			this.iface.__commonBufferChanged(fN, miForm);
			break;
...
/*function klo_habilitarPorIvaIncluido()
{
	var cursor:FLSqlCursor = this.cursor();
