...
class klo extends ivaIncluido 
{
	...
	//function aplicarTarifaLinea(codTarifa:String):Boolean {
	//	return this.ctx.klo_aplicarTarifaLinea(codTarifa);
	//}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function klo_codigodebarrasSet()
{
	...
	var refArticulo:String = util.sqlSelect( "codigosdebarras", "referencia", "codigodebarras = '" + this.child("fdbCodBarras").value()+"'");
	var cantArticulo:Number = util.sqlSelect( "codigosdebarras", "unidades", "codigodebarras = '" + this.child("fdbCodBarras").value()+"'");

	if (refArticulo)  {
		this.child("fdbReferencia").setValue(refArticulo);
		this.iface.txtCanArticulo.setText(cantArticulo);
		this.child("txtCanArticulo").setFocus();
	}
}
...
function klo_bufferChanged(fN:String)
{
	...
	switch (fN) {
		case "codbarrasLinea":
			case "codbarras": {
				this.iface.codigodebarrasSet();
				break;
			}
			case "cifnif": {
				this.child("fdbTarifa").setValue(this.iface.calculateField("codtarifa"));
...
function klo_calculateField(fN:String):String
{
	...
	switch (fN) {
		case "codtarifa": {
			...
			var codGrupo:String = util.sqlSelect("clientes","codgrupo","codcliente = '"+codCliente+"'");
			if (codGrupo) {
				valor = util.sqlSelect("gruposclientes","codtarifa","codgrupo = '"+codGrupo+"'");
			}
			if (!valor || valor == "") {
				valor = util.sqlSelect("tpv_datosgenerales","tarifa","1=1");
			}
			break;
		}
		case "pvparticulo": {
...
/*function klo_aplicarTarifaLinea(codTarifa:String):Boolean
{
	debug("KLO--> Hola 1");
