...
//////////////////////////////////////////////////////////////////
//// KLO /////////////////////////////////////////////////////
class klo extends tpvtactIvainc {
	function klo( context ) { tpvtactIvainc( context ); } 
	function init() {
		return this.ctx.klo_init();
	}
	function clickedTabla(fil:Number,col:Number) {
		return this.ctx.klo_clickedTabla(fil,col);
	}
	function construirArrayArticulos(nivel:String) {
		return this.ctx.klo_construirArrayArticulos(nivel);
	}
	function datosLineaVenta(referencia:String, cantidad:Number,comanda:Number):Boolean {
		return this.ctx.klo_datosLineaVenta(referencia, cantidad,comanda);
	}
}
//// KLO /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function klo_init()
{
	...
	if (this.iface.secuenciaLinea_ == "Cantidad_Referencia") {
		this.iface.modificarCantidad(10);	// La cantidad siempre será 0.
		this.child("tbnOK").close();
	}
}

function klo_clickedTabla(fil:Number,col:Number)
{
	var indice:Number = this.iface.dameIndiceCelda(fil, col);
	if (indice < 0) {
		return false;
...
function klo_construirArrayArticulos(nivel:String)
{
	debug("KLO --> construirArrayArticulos()");
	this.iface.arrayArticulos[nivel] = [];
	this.iface.paginas[nivel] = 0;
	var i:Number = 0;
...
/** |D Establece los datos de la línea de ventas a crear mediante la inserción rápida
\end */
function klo_datosLineaVenta(referencia:String, cantidad:Number,comanda:Number):Boolean
{
	var util:FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.cursor();

	debug("KLO--> datosLineaVenta()");
	...
	this.iface.curLineas.setValueBuffer("codimpuesto",  codImpuesto);
	this.iface.curLineas.setValueBuffer("iva", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("iva", this.iface.curLineas));
	var iva:Number = parseFloat(this.iface.curLineas.valueBuffer("iva"));

	var pvp:Number;
	if(this.iface.precioLibre && this.iface.precioLibre != 0) {
		pvp = this.iface.precioLibre;
		if (ivaIncluido) {
			this.iface.curLineas.setValueBuffer("pvpunitario", parseFloat(pvp/((iva/100)+1))); 
			this.iface.curLineas.setValueBuffer("pvpunitarioiva", pvp); 
		}
		else {
			this.iface.curLineas.setValueBuffer("pvpunitario", pvp); 
			this.iface.curLineas.setValueBuffer("pvpunitarioiva", parseFloat(pvp*((iva/100)+1))); 
		}
	}
	else {
		//--- Adaptación para rellenar todos los campos de precio teniendo en cuenta si el artículo es IVA incluido.
		//debug("KLO--> ivaIncluido: "+ivaIncluido);
		if (this.iface.curLineas.valueBuffer("ivaincluido")) {
			//debug("KLO--> dentro de IVA incluido");
			this.iface.curLineas.setValueBuffer("pvpunitarioiva", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("pvpunitarioiva", this.iface.curLineas));
			this.iface.curLineas.setValueBuffer("pvpunitario", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("pvpunitario2", this.iface.curLineas));
			//debug("KLO--> pvpunitarioiva: "+this.iface.curLineas.valueBuffer("pvpunitarioiva"));
		} else {
			this.iface.curLineas.setValueBuffer("pvpunitario", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("pvpunitario", this.iface.curLineas));
			this.iface.curLineas.setValueBuffer("pvpunitarioiva", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("pvpunitarioiva2", this.iface.curLineas));
		}
	...
	//--- Asignar coste de la línea.
	var fechaFin:Date = util.sqlSelect("tpv_comandas", "fecha", "idtpv_comanda = "+comanda);
	var codAlmacen:String = util.sqlSelect("tpv_comandas", "codalmacen", "idtpv_comanda = "+comanda);
	this.iface.curLineas.setValueBuffer("coste", flfactalma.iface.pub_costeEnUnaFecha(referencia, codAlmacen, fechaFin));
	//--------
