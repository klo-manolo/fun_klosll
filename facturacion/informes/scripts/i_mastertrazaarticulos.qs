...
class klo extends oficial {
	...
	function rellenarDatos(cursor:FLSqlCursor) {
		return this.ctx.klo_rellenarDatos(cursor);
	}
}
//// KLO /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function klo_lanzar()
{
	...
	var codFamilia:String = cursor.valueBuffer("codfamilia");
	//KLO
	var codSeccion:String = cursor.valueBuffer("codseccion");
	...
	//KLO
	if (codSeccion)
		where += " AND articulos.codseccion = '" + codSeccion + "'";
	//--
	if (codFamilia)
		where += " AND articulos.codfamilia = '" + codFamilia + "'";
	//KLO
	if (codFamilia)
		where += " AND articulos.codsubfamilia = '" + codSubfamilia + "'";
	//--
...
function klo_rellenarDatos(cursor:FLSqlCursor)
{
	if (!cursor)
		cursor = this.cursor();
	...
	var proveedorHasta:String = cursor.valueBuffer("proveedorhasta");
	// KLO
	var agenteTpvDesde:String = cursor.valueBuffer("agentetpvdesde");
	var agenteTpvHasta:String = cursor.valueBuffer("agentetpvhasta");
	//--
	...
	// KLO
	if (agenteTpvDesde)
		where += " AND t.codtpv_agente >= '" + agenteTpvDesde + "'";
	if (agenteTpvHasta)
		where += " AND t.codtpv_agente <= '" + agenteTpvHasta + "'";
	if (agenteTpvDesde || agenteTpvHasta)
		where += " AND t.codtpv_agente IS NOT NULL";
	//--
	...
	q.setFrom("facturascli f " +
			"INNER JOIN lineasfacturascli l ON f.idfactura = l.idfactura " +
			"LEFT JOIN albaranescli a ON l.idalbaran = a.idalbaran " +
			"LEFT JOIN tpv_comandas t ON l.idfactura = t.idfactura");
	q.setWhere(where);
	debug(q.sql());
