...
class klo extends oficial
{
	...
	function costeArticulo(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_costeArticulo(nodo, campo);
	}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
//// KLO /////////////////////////////////////////////////
function klo_costeArticulo(nodo:FLDomNode, campo:String):String
{
	debug("Coste unidad klo ");
	var util:FLUtil = new FLUtil;
	var tipoValoracion:String = nodo.attributeValue("almacenes.tipovaloracion");
	var campoRed:String;
	var tablaRed:String;
	debug("tipoValoracion klo " + tipoValoracion);
	var valor:Number;
	switch (tipoValoracion) {
		case "Coste medio": {
			valor = nodo.attributeValue("articulos.costemedio")
	...
	if (campo == "total") {
		var cantidad:Number = parseFloat(nodo.attributeValue("stocks.cantidad"));
		valor = parseFloat(valor) * parseFloat(cantidad);
		debug("valor antes pridahoalm" + valor);
		debug("this.iface.totalAlmacen_ klo" + this.iface.totalAlmacen_);
		debug("this.iface.totalInforme_ klo" + this.iface.totalInforme_);
		if (isNaN(this.iface.totalAlmacen_)) {
			this.iface.totalAlmacen_ = 0;
		}
		this.iface.totalAlmacen_ += parseFloat(valor);
