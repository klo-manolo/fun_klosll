...
/////////////////////////////////////////////////////////////////
//// KLO /////////////////////////////////////////////////////
class klo extends recibosProv 
{
	var idInforme:Number;

	function klo( context ) { recibosProv ( context ); }
	
	function desgloseIva(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_desgloseIva(nodo, campo);
	}
	function desgloseBaseImponible(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_desgloseBaseImponible(nodo, campo);
	}
	function desgloseRecargo(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_desgloseRecargo(nodo, campo);
	}
	function desgloseTotal(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_desgloseTotal(nodo, campo);
	}
	
	function setIdInforme(idInforme:Number) {
		return this.ctx.klo_setIdInforme(idInforme);
	}
	function encabezadoInforme(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_encabezadoInforme(nodo, campo);
	}
	function desgloseImpuestos(nodo:FLDomNode, campo:String):String {
		return this.ctx.klo_desgloseImpuestos(nodo, campo);
	}
	function lanzarJRXML(informe:String,modelo:String,argumentos:String) {
		return this.ctx.klo_jasper_lanzarJRXML(informe,modelo,argumentos);
	}
	function lanzarInforme(cursor:FLSqlCursor, nombreInforme:String, orderBy:String, groupBy:String, etiquetas:Boolean, impDirecta:Boolean, whereFijo:String, nombreReport:String, numCopias:Number, impresora:String, pdf:Boolean) {
		return this.ctx.klo_jasper_lanzarInforme(cursor, nombreInforme, orderBy, groupBy, etiquetas, impDirecta, whereFijo, nombreReport, numCopias, impresora, pdf);
	}
	function sysexec(comando:String):Array {
		return this.ctx.klo_jasper_sysexec(comando);
	}
}
//// KLO /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
//// PUB_KLO //////////////////////////////////////////////////
class pubKlo extends pubDtoEspecial {
	function pubKlo( context ) { pubDtoEspecial( context ); }
	function pub_desgloseImpuestos(nodo:FLDomNode, campo:String):String {
		return this.desgloseImpuestos(nodo, campo);
	}
}
...
/** En los procedimientos de desglose solo se cambia el orden del cursor
para que salga primero el IVA 16, 7 y 4 en ese orden. */
function klo_desgloseIva(nodo:FLDomNode, campo:String):String
{
	if (!this.iface.variosIvas_)
		return "";
	...
	with (qryIvas) {
		...
		setSelect("totaliva");
		setFrom(tabla);
		setWhere(campoClave + " = " + idDocumento + " ORDER BY iva DESC");
	}
	if (!qryIvas.exec())
		return false;
	var listaIvas:String = "";
...
function klo_desgloseBaseImponible(nodo:FLDomNode, campo:String):String
{
	if (!this.iface.variosIvas_)
		return "";
...
function klo_desgloseRecargo(nodo:FLDomNode, campo:String):String
{
	if (!this.iface.variosIvas_)
		return "";
...
function klo_desgloseTotal(nodo:FLDomNode, campo:String):String
{
	if (!this.iface.variosIvas_)
		return "";
...
function klo_setIdInforme(idInforme)
{
	this.iface.idInforme = idInforme;
}

function klo_encabezadoInforme(nodo:FLDomNode, campo:String):String
{
	var util:FLUtil = new FLUtil;
	
	var qry:FLSqlQuery = new FLSqlQuery();
	qry.setTablesList("i_tarifasarticulos");
	qry.setSelect("d_secciones_codseccion, h_secciones_codseccion, d_familias_codfamilia, h_familias_codfamilia, d_subfamilias_codsubfamilia, h_subfamilias_codsubfamilia, d_articulos_referencia, h_articulos_referencia, codtarifa1, codtarifa2");
	qry.setFrom("i_tarifasarticulos");
	qry.setWhere("id = " + this.iface.idInforme);
	debug("------ CONSULTA KLO -------" + qry.sql());
	...
	switch (campo) {
		case "seccion":
			if (qry.value("d_secciones_codseccion") && qry.value("h_secciones_codseccion")) {
				var nombre1:String = util.sqlSelect("secciones", "descripcion", "codseccion = '"+qry.value("d_secciones_codseccion")+"'");
				var nombre2:String = util.sqlSelect("secciones", "descripcion", "codseccion = '"+qry.value("h_secciones_codseccion")+"'");
				texto += util.translate("scripts","SECCIONES: ") + qry.value("d_secciones_codseccion")+" - " + nombre1 + " hasta " + qry.value("h_secciones_codseccion") + " - " + nombre2;
			} else {
				texto = "SECCIONES: Todo.";
			}
			break;
		case "familia":
			if (qry.value("d_familias_codfamilia") && qry.value("h_familias_codfamilia")) {
				var nombre1:String = util.sqlSelect("familias", "descripcion", "codfamilia = '"+qry.value("d_familias_codfamilia")+"'");
				var nombre2:String = util.sqlSelect("familias", "descripcion", "codfamilia = '"+qry.value("h_familias_codfamilia")+"'");
				texto += util.translate("scripts","FAMILIAS: ") + qry.value("d_familias_codfamilia")+" - " + nombre1 + " hasta " + qry.value("h_familias_codfamilia") + " - " + nombre2;
			} else {
				texto = "FAMILIAS: Todo.";
			}
			break;
		case "subfamilia":
			if (qry.value("d_subfamilias_codsubfamilia") && qry.value("h_subfamilias_codsubfamilia")) {
				var nombre1:String = util.sqlSelect("subfamilias", "descripcion", "codsubfamilia = '"+qry.value("d_subfamilias_codsubfamilia")+"'");
				var nombre2:String = util.sqlSelect("subfamilias", "descripcion", "codsubfamilia = '"+qry.value("h_subfamilias_codsubfamilia")+"'");
				texto += util.translate("scripts","SUBFAMILIAS: ") + qry.value("d_subfamilias_codsubfamilia")+" - " + nombre1 + " hasta " + qry.value("h_subfamilias_codsubfamilia") + " - " + nombre2;
			} else {
				texto = "SUBFAMILIAS: Todo.";
			}
			break;
		case "referencia":
			if (qry.value("d_articulos_referencia") && qry.value("h_articulos_referencia")) {
				var nombre1:String = util.sqlSelect("articulos", "descripcion", "referencia = '"+qry.value("d_articulos_referencia")+"'");
				var nombre2:String = util.sqlSelect("articulos", "descripcion", "referencia = '"+qry.value("h_articulos_referencia")+"'");
				texto += util.translate("scripts","ARTÍCULOS: ") + qry.value("d_articulos_referencia")+" - " + nombre1 + " hasta " + qry.value("h_articulos_referencia") + " - " + nombre2;
			} else {
				texto = "ARTÍCULOS: Todo.";
			}
			break;
		case "tarifa1":
			texto = util.sqlSelect("tarifas", "nombre", "codtarifa = '"+qry.value("codtarifa1")+"'");
			if (!texto)
				texto = "";
			break;
		case "tarifa2":
			texto = util.sqlSelect("tarifas", "nombre", "codtarifa = '"+qry.value("codtarifa2")+"'");
			if (!texto)
				texto = "";
			break;
	}
...
function klo_desgloseImpuestos(nodo:FLDomNode, campo:String):String
{
	if (!this.iface.variosIvas_)
		return "";
	...
	with (qryIvas) {
		setTablesList(tabla);
		setSelect("totaliva, totalrecargo");
		setFrom(tabla);
		setWhere(campoClave + " = " + idDocumento + " ORDER BY iva DESC");
	}
	if (!qryIvas.exec())
		return false;
	var listaImpuestos:String = "";
	while (qryIvas.next()) {
		if (listaImpuestos != "")
			listaImpuestos += "\n";
		if (qryIvas.value("totalrecargo") == 0)
			listaImpuestos += util.roundFieldValue(qryIvas.value("totaliva"), tabla, "iva");
		else
			listaImpuestos += util.roundFieldValue(qryIvas.value("totaliva")+qryIvas.value("totalrecargo"), tabla, "iva");
	}
	return listaImpuestos;
}

function klo_jasper_sysexec(comando:String):Array
{
	var res:Array = [];
...
function klo_jasper_lanzarJRXML(informe:String,modelo:String,argumentos:String)
{
	var util:FLUtil = new FLUtil();    
...
@param	whereFijo: Sentencia where que debe preceder a la sentencia where calculada por la función
\end */
function klo_jasper_lanzarInforme(cursor:FLSqlCursor, nombreInforme:String, orderBy:String, groupBy:String, etiquetas:Boolean, impDirecta:Boolean, whereFijo:String, nombreReport:String, numCopias:Number, impresora:String, pdf:Boolean)
{
